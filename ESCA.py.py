...
def main():
    ...
    if st.button("Fetch Data"):
        ...
        ax.plot(test_data.index[window_size:], actual, label="Actual Prices", color="blue")
        ax.plot(test_data.index[window_size:], predictions, label="Predicted Prices", color="orange")
        ...
        # Display Next-Day, 1-Week, and 1-Month Predictions
        st.subheader("Future Predictions")
        next_day_price = future_prices[0][0]
        one_week_price = future_prices[6][0] if len(future_prices) >= 7 else None
        one_month_price = future_prices[29][0] if len(future_prices) >= 30 else None

        st.write(f"**Next-Day Prediction:** {next_day_price:.4f}")
        if one_week_price:
            st.write(f"**1-Week Prediction:** {one_week_price:.4f}")
        if one_month_price:
            st.write(f"**1-Month Prediction:** {one_month_price:.4f}")

        # Show probability of hitting a target price
        st.subheader("Probability of Target Price")
        target_price = st.number_input("Enter Target Price:", value=1.2)
        std_dev = np.std(actual - predictions)
        prob = norm.cdf((target_price - next_day_price) / std_dev) * 100

        st.write(f"**Probability of hitting target price {target_price:.4f}:** {prob:.2f}%")
