...
def main():
    ...
    if st.button("Fetch Data"):
        ...
        probabilities = np.array(calculate_prediction_probability(future_prices, actual[-1][0], std_dev)).flatten()
        ...
        st.write(f"**Next Week Predictions:** {next_week_prices.tolist()} (Average Probability: {np.mean(probabilities[:7]):.2f}%)")
        st.write(f"**Next Month Predictions:** {next_month_prices.tolist()} (Average Probability: {np.mean(probabilities):.2f}%)")
